...
sub run {
    ...
    my @hdblcm_args = qw(--action=install --components=server --shell=/bin/csh --hostname=$HOSTNAME --sid=ABC --number=00 --password=Linux1234 --system_user_password=Linux1234 --sapadm_password=Linux1234 --batch --ignore=check_signature_file);

    my $cmd = join(' ', $hdblcm, @hdblcm_args);
    record_info 'hdblcm command', $cmd;
    assert_script_run $cmd, $tout;

    @hdblcm_args = qw(--action=install --components=server --shell=/bin/csh --hostname=$HOSTNAME --sid=DEF --number=10 --password=Linux1234 --system_user_password=Linux1234 --sapadm_password=Linux1234 --batch --ignore=check_signature_file);

    $cmd = join(' ', $hdblcm, @hdblcm_args);
    record_info 'hdblcm command', $cmd;
    assert_script_run $cmd, $tout;
